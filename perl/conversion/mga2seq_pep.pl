...
open(NOPREDICT, ">".$options{outdir}."/".$options{prefix}.".no_prediction.list") or die("Could not open file $options{outdir}/$options{prefix}.no_prediction.list");
...
sub check_parameters {
    ...
    unless ( $options{input} && $options{mga} && $options{prefix} && $options{outdir}) {
		die("Missing input, plesae read perldoc $0\n\n");
		exit(1);
    }
