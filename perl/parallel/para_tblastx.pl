...
# MANUAL FOR para_tblastx.pl

=pod

=head1 NAME

para_tblastx.pl -- embarasingly parallel tBLASTx

=head1 SYNOPSIS

 para_tblastx.pl -query /path/to/infile.fasta -db /path/to/db -out /path/to/output.btab -evalue 1e-3 -outfmt 6 -threads 1
                     [--help] [--manual]
...
if ($threads == 1) {
	...
	my $blast_exe = "$program -query $tmp_file/split-$i.fsa -db $db -out $tmp_file/btab_splits/split.$i.btab -outfmt $outfmt -evalue $evalue -num_threads 1 -max_target_seqs 100000";
